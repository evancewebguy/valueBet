defmodule ValueBet.Accounts do
  @moduledoc """
  ...
  def list_users() do
    ...
    Repo.all(
      from u in ValueBet.Accounts.User,
        join: ur in assoc(u, :user_roles),
        join: r in assoc(ur, :role),
        preload: [user_roles: {ur, role: r}]
    )
  end


  def get_user_with_roles_and_permissions(user_id) do
    Repo.one(
      from u in User,
        where: u.id == ^user_id,
        join: ur in assoc(u, :user_roles),
  ...
  def add_permission_to_role(role_id, permission_id) do
    ...
    case Repo.get_by(RolePermission, role_id: role_id, permission_id: permission_id) do
      ...
      _role_permission ->
        # Permission already exists, no need to add
        {:ok, nil}
    end
  end
  ...
  def update_user_role(user, role) do
    user
    |> Ecto.Changeset.change(role: role)  # Assuming there's a 'role' field on the user
    |> Repo.update()
  end
  ...
  @doc """
  assign permissions to a role.- assign_permissions_to_role

  """
  def assign_permissions_to_role(role_id, permission_ids) do
    permissions =
    ...
    case Repo.insert_all(RolePermission, permissions, on_conflict: :nothing) do
      {count, _} when count > 0 -> {:ok, permissions}
      _ -> {:error, "Failed to assign permissions to the role"}
    end
  end
